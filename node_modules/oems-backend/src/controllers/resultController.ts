...
import { mockDb, Result, NotificationItem } from '../services/mockDb.js';
import { AuthRequest } from '../middleware/authMiddleware.js';
import { sendEmail } from '../config/resend.js';

export async function publishResults(req: AuthRequest, res: Response) {
  ...
  const submissions = mockDb.examSubmissions.filter((s) => s.exam_id === examId && s.submitted_at);
  ...
  mockDb.results = mockDb.results.filter((r) => r.exam_id !== examId);
  ...
  submissions.forEach((sub, index) => {
    ...
    mockDb.results.push(resultRecord);
  ...
  mockDb.logAudit('PUBLISH_RESULTS', 'EXAM', examId, { count: submissions.length }, req.user?.id, req.user?.email);
...
export async function getResults(req: AuthRequest, res: Response) {
  ...
  const enriched = list.map((r) => {
    const exam = mockDb.exams.find((e) => e.id === r.exam_id);
    const student = mockDb.profiles.find((p) => p.id === r.student_id);
    return {
      ...r,
...
export async function getResultDetail(req: AuthRequest, res: Response) {
  ...
  const result = mockDb.results.find((r) => r.exam_id === examId && r.student_id === targetStudentId);
  if (!result) {
    return res.status(404).json({ error: 'Result record not found or not published yet.' });
  }

  const exam = mockDb.exams.find((e) => e.id === examId);
  const student = mockDb.profiles.find((p) => p.id === targetStudentId);
  const submission = mockDb.examSubmissions.find((s) => s.id === result.submission_id);

  // Detailed answer key review
  const examQuestions = mockDb.examQuestions.filter((eq) => eq.exam_id === examId);
  const answerReview = examQuestions.map((eq) => {
    const q = mockDb.questions.find((question) => question.id === eq.question_id);
    const options = mockDb.questionOptions.filter((o) => o.question_id === eq.question_id);
    const studentAns = mockDb.studentAnswers.find(
      (a) => a.submission_id === submission?.id && a.question_id === eq.question_id
    );
...
export async function exportResultsCSV(req: AuthRequest, res: Response) {
  const { examId } = req.params;
  const exam = mockDb.exams.find((e) => e.id === examId);
  if (!exam) {
    return res.status(404).json({ error: 'Exam not found' });
  ...
  const results = mockDb.results.filter((r) => r.exam_id === examId);
